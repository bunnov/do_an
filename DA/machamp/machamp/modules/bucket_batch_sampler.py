...
def lazy_groups_of(indices: Iterable[A], group_size: int, sizes: Iterable[A], batch_size, max_tokens) -> Iterator[
    List[A]]:
    """
    ...
    for indice, size in zip(indices, sizes):
        ...
        if (len(cur_batch) == batch_size and batch_size != -1) or (
                max_tokens != -1 and cur_size + size[0] > max_tokens):
            yield cur_batch
            cur_batch = [indice]
            cur_size = size[0]
