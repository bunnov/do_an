...
def create_token_index(curr_partition, tokenizer, tokenizer_name):
    ...
    for dataset_name in DATASET_NAMES:
        with open(os.path.join("data", dataset_name + ".train"), "r",  encoding="utf8") as f:
            for line in f:
...
if __name__ == "__main__":
    ...
    for curr_partition in partitions:
        ...
        output_file_avg = open(output_filepath_avg, "w", encoding="utf8")

        is_header = True
        max_examples = 10

        for line in open(contributions_filepath, "r", encoding="utf8"):
            if is_header:
